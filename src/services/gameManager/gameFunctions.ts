...
import { Ace, Card, CardNumber, Eight, Five, Four, Jack, King, Nine, Queen, Seven, Six, Suit, Ten, Three, Two } from "../../models/card";
...
export function IsNominationCard(card: Card, discardPile: Card[]) {
    ...
    let isEightOnAce = card.card === CardNumber.Eight && topDiscard?.card === CardNumber.Ace && !(topDiscard as Ace)?.isOne;
...
export function createShuffledDeck() {
  ...
  export function canPlayCard(player: Player, cards: Card[], discardPile: Card[], onFailCallback: (message: string) => void) {
